...
import ig3.iscae.service.DEService;
...
DEService DE=new DEService();


@GET
@Path("/cnx")
public DirecteurEtude login(@QueryParam("username") String username,@QueryParam("password") String password) {
return DE.connexion();
}
   ...
   @Path("/cnx/matieres")
	...
	public Crenaux ajoutCrenaux(@QueryParam("jour") String jour,@QueryParam("debut") String debut,@QueryParam("fin") String fin,
	  ...
	  if(Memoire.getCrenaux().isEmpty()) {
		  ...
		  return DE.addCrenaux(c);
	  }
       return DE.addCrenaux(c);// un crenaux pas cours :D
	}
